...
export default function admin({
   ...
   return (
		<main className=" grid grid-cols-4 gap-4 w-full p-4 auto-rows-auto">
						...
						{rices.map((rice) => (
							<tr key={rice.id} className="border-b border-gray-400/20 hover:bg-neutral-400/20">
								...
								<td className="flex-row flex justify-center">
									<p className=" text-xl p-1 rounded-lg hover:text-blue-500 hover:scale-110 font-semibold cursor-pointer" ><FontAwesomeIcon icon={faEdit} onClick={()=> {setShowEditModal(true); setSelectedRice(rice)}} /></p>
			...
			{/* User Section */}
			<section className="backdrop-blur-2xl bg-khaki border-l-5 border-brown p-2 shadow-2xl rounded-lg w-full h-full flex col-span-4 flex-col" >
				<p className="text-2xl opacity-70 w-full text-center mb-3">USER</p><p onClick={()=> setShowAddRiceModal(true)}  className="absolute bg-brown right-5 top-3 text-offwhite p-1 w-1/7 rounded-lg hover:scale-105 active:scale-95 font-semibold text-center cursor-pointer" >Add User</p>
