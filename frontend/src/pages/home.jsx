		...
		<main className="gap-4 w-full p-4 h-full relative flex flex-col box-border">
			...
			<section className="w-full flex gap-6 h-full flex-wrap p-2" >
				...
				{filtered_rices.map((filtered_rice)=> (
					<div key={filtered_rice.id} onClick={()=> {navigate('/riceInfo'); set_selected_rice(filtered_rice)}}  className="w-[32%] h-fit bg-khaki rounded-lg flex justify-start items-start relative cursor-pointer">
						...
						<div className="w-1/2 right-0 h-full p-2 relative gap-1 flex flex-col justify-start">
							...
							<p className="opacity-80">{filtered_rice.is_25kg ? 25 : filtered_rice.is_50kg ? 50 : filtered_rice.weight_kg} kg</p>
