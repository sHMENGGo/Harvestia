...
// Path of backend folder
const dirName = path.dirname(fileURLToPath(import.meta.url))

// Expose everything inside images folder to see it in ":5000/images/"
...
const riceImagesDir = path.join(imagesDir, 'riceImage')
const userImagesDir = path.join(imagesDir, 'userImage')

// Configure rice image storage in the system folder
const riceImageStorage = multer.diskStorage({
  destination: (req, file, cb)=> {cb(null, riceImagesDir)},
  // unique file name
  filename: (req, file, cb)=> {
    const inputRice = req.body.inputRice
    const uniqueSuffix = Date.now() + '-' + Math.round(Math.random()* 1E9)
    cb(null, inputRice + '-' + uniqueSuffix + path.extname(file.originalname))
  }
})
const riceImageUpload = multer({storage: riceImageStorage})
...
app.post('/api/addRice', verifyToken, verifyAdmin, riceImageUpload.single('image'), async (req, res)=> {
  ...
  try {
    ...
    await prisma.Rice.create({data: {
      ...
      imagePath: `/images/riceImage/${fileName}`
    }})
...
app.get('/api/getRices', verifyToken, async (req, res)=>{
  ...
  } catch(err) {res.status(500).json({ message: 'Fetching Rices from database failed ', err})}
})
